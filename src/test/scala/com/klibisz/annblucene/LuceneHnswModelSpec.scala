...
class LuceneHnswModelSpec extends AnyFreeSpec with Matchers with GloveVectors {
  ...
  "sanity check on glove vectors" - {
    ...
    "end-to-end" in {
      ...
      words.map(_.reverse) shouldBe Success(Vector("president", "minister", "secretary", "chairman", "leader"))
    }
